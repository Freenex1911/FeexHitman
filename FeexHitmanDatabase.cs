...
namespace Freenex.FeexHitman
{
    public class DatabaseManager
    {
        ...
        private MySqlConnection CreateConnection()
        {
            ...
            try
            {
                if (FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabasePort == 0) FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabasePort = 3306;
                connection = new MySqlConnection(String.Format("SERVER={0};DATABASE={1};UID={2};PASSWORD={3};PORT={4};", FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseAddress, FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseName, FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseUsername, FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabasePassword, FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabasePort));
            }
            catch (Exception ex)
            {
        ...
        public bool CheckExists(CSteamID id)
        {
            try
            {
                ...
                int exists = 0;
                command.CommandText = "SELECT COUNT(1) FROM `" + FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseTableName + "` WHERE `steamId` = '" + id.ToString() + "';";
                connection.Open();
        ...
        public string GetBountyList()
        {
            ...
            try
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT * FROM (SELECT * FROM `" + FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseTableName + "` WHERE `lastUpdated` ORDER BY `lastUpdated` DESC LIMIT " + FeexHitman.Instance.Configuration.Instance.CommandListMaximum + ") AS tbl ORDER BY `lastUpdated` ASC;";
                connection.Open();
        ...
        public int GetBountyCount()
        {
            ...
            try
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SELECT COUNT(*) FROM `" + FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseTableName + "`;";
                connection.Open();
                object result = command.ExecuteScalar();
        ...
        internal void CreateCheckSchema()
        {
            try
            {
                MySqlConnection connection = CreateConnection();
                MySqlCommand command = connection.CreateCommand();
                command.CommandText = "SHOW TABLES LIKE '" + FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseTableName + "';";
                connection.Open();
                object test = command.ExecuteScalar();

                if (test == null)
                {
                    command.CommandText = "CREATE TABLE `" + FeexHitman.Instance.Configuration.Instance.FeexHitmanDatabase.DatabaseTableName + "` (`steamId` varchar(32) NOT NULL, `bounty` decimal(15,2) NOT NULL DEFAULT '25.00', `lastDisplayName` varchar(32) NOT NULL, `lastUpdated` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00', PRIMARY KEY (`steamId`));";
